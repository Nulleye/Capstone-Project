package com.nulleye.yaaa.util.external;
...
/**
 * com.afollestad.materialdialogs.folderselector.FileChooserDialogEx
 *
 * Mod to support several mimeTypes at once
 *
 */

public class FileChooserDialogEx extends DialogFragment implements MaterialDialog.ListCallback {
    ...
    public interface FileCallback {
        void onFileSelection(@NonNull FileChooserDialogEx dialog, @NonNull File file);
    }

    public FileChooserDialogEx() {
    }
    ...
    //MOD: mimeType can be :"type1;type2; ... ;typeN"
    File[] listFiles(String mimeType) {
        File[] contents = parentFolder.listFiles();
        List<File> results = new ArrayList<>();
        if (contents != null) {
            MimeTypeMap mimeTypeMap = MimeTypeMap.getSingleton();
            String[] mimes = mimeType.split(";");   //MOD
            for (File fi : contents) {
                if (fi.isDirectory()) {
                    ...
                    for(String mime:mimes)
                        if (FnUtil.fileIsMimeType(fi, mime)) {
                            results.add(fi);
                            break;
                        }
                }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (getArguments() == null || !getArguments().containsKey("builder"))
            throw new IllegalStateException("You must create a FileChooserDialogEx using the Builder.");
    ...
    public static class Builder implements Serializable {
        ...
        @NonNull
        public FileChooserDialogEx build() {
            FileChooserDialogEx dialog = new FileChooserDialogEx();
        ...
        @NonNull
        public FileChooserDialogEx show() {
            FileChooserDialogEx dialog = build();
