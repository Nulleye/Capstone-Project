 ...
 *
 * CAP - switched Inner and Outer hour texts
 *       RadialTextsView.setSelection has degrees and isInner information to clip text in selection
 *       circle.
 */
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
    ...
    public void initialize(Context context, TimePickerController timePickerController,
        ...
        for (int i = 0; i < 12; i++) {
            //CAP - switch inner and outer hours
            innerHoursTexts[i] = is24HourMode?
...
//            innerHoursTexts[i] = String.format(Locale.getDefault(), "%d", hours[i]);
            hoursTexts[i] = String.format(Locale.getDefault(), "%d", hours[i]);
            minutesTexts[i] = String.format(Locale.getDefault(), "%02d", minutes[i]);
            secondsTexts[i] = String.format(Locale.getDefault(), "%02d", seconds[i]);
        }

        //CAP - move the commented code here as RadialTextsView.setSelection now needs the degrees
        ...
        //CAP - Add degrees and isInner to RadialTextsView.setSelection
        ...
        mMinuteRadialTextsView.setSelection(initialTime.getMinute(), minuteDegrees, false);
        mMinuteRadialTextsView.invalidate();
        mSecondRadialTextsView.initialize(context, secondsTexts, null, mController, secondValidator, false);
        mSecondRadialTextsView.setSelection(initialTime.getSecond(), secondDegrees, false);
    ...
    private void reselectSelector(Timepoint newSelection, boolean forceDrawDot, int index) {
        switch(index) {
            case HOUR_INDEX:
                ...
                mHourRadialTextsView.setSelection(hour, degrees, isInnerCircle);
                ...
                if(newSelection.getMinute() != mCurrentTime.getMinute()) {
                    ...
                    mMinuteRadialSelectorView.setSelection(minDegrees, false, forceDrawDot);
                    mMinuteRadialTextsView.setSelection(newSelection.getMinute(), minDegrees, false);
                }
                ...
                if(newSelection.getSecond() != mCurrentTime.getSecond()) {
                    ...
                    mSecondRadialTextsView.setSelection(newSelection.getSecond(), secDegrees, false);
                }
                break;
            case MINUTE_INDEX:
                ...
                mMinuteRadialTextsView.setSelection(newSelection.getMinute(), degrees, false);
                // If we rounded the seconds, reposition the secondSelector too.
                if(newSelection.getSecond() != mCurrentTime.getSecond()) {
                    int secDegrees = newSelection.getSecond()*360/60;
                    mSecondRadialSelectorView.setSelection(secDegrees, false, forceDrawDot);
            ...
            case SECOND_INDEX:
                ...
                mSecondRadialTextsView.setSelection(newSelection.getSecond(), degrees, false);
        }
