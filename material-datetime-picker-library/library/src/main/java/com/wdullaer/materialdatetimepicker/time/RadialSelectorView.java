 ...
 *
 * CAP - Correction to draw small white circle when selection is not multiple of 30 degrees
 *       It should work but the previous code didn't
 *
 */
public class RadialSelectorView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        // CAP - I think the commented code is wrong
        if (mSelectionDegrees % 30 != 0) {
            // We're not on a direct tick (or we've been told to draw the dot anyway).
            int col = mPaint.getColor();
            mPaint.setColor(ContextCompat.getColor(getContext(), R.color.mdtp_white));
            canvas.drawCircle(pointX, pointY, (mSelectionRadius * 2 / 14), mPaint);
            mPaint.setColor(col);
        }
