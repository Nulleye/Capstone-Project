 ...
 *
 * CAP - RadialTextsView.setSelection has degrees and isInner information to clip text in selection
 *       circle.
 *       Draw method now draws text twice, one for black and one for white clipped to selection circle
 *
 */
public class RadialTextsView extends View {
    ...
    //CAP - current radial selector info to clip white text
    private int mSelectionRadius;
    private double mSelectionRadians;
    private float mCurrentRadiusMultiplier;
    //CAP - radial selector radious and clip helper
    ...
    public void initialize(Context context, String[] texts, String[] innerTexts,
        ...
        //CAP - get current selection radius
     ...
     * @param selectionDegrees CAP - get info about current selector degrees
     * @param isInnerCircle CAP - which band is currently selected? normal or innner
     */
    protected void setSelection(int selection, int selectionDegrees, boolean isInnerCircle) {
        this.selection = selection;
        //CAP - calculate current selection radians and multiplier (normal or inner)
        mSelectionRadians = selectionDegrees * Math.PI / 180;
        if (mHasInnerCircle && isInnerCircle) {
            mCurrentRadiusMultiplier = mInnerNumbersRadiusMultiplier;
        } else mCurrentRadiusMultiplier = mNumbersRadiusMultiplier;
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (!mDrawValuesReady) {
            ...
            //CAP - calculate selection radious
        ...
        //CAP - calculate the clip circle
        int lineLength = (int) (mCircleRadius * mCurrentRadiusMultiplier * mAnimationRadiusMultiplier);
        int pointX = mXCenter + (int) (lineLength * Math.sin(mSelectionRadians));
        int pointY = mYCenter - (int) (lineLength * Math.cos(mSelectionRadians));
        path.reset();
        path.addCircle(pointX,pointY,mSelectionRadius, Path.Direction.CW);
        canvas.clipPath(path);
        int cl = mPaint.getColor();
        mPaint.setColor(Color.WHITE);
        //TOCH Optimize - Draw only the affected white texts
        drawTexts(canvas, mTextSize, mTypefaceLight, mTexts, mTextGridWidths, mTextGridHeights);
        if (mHasInnerCircle) {
            drawTexts(canvas, mInnerTextSize, mTypefaceRegular, mInnerTexts,
                    mInnerTextGridWidths, mInnerTextGridHeights);
    ...
    private Paint[] assignTextColors(String[] texts) {
        ...
        for(int i=0;i<texts.length;i++) {
            ...
            //CAP - Disable this code, is useless with the new clipping mode
...
//            if(mValidator.isValidSelection(text))
                paints[i] = mPaint;
//            else paints[i] = mInactivePaint;
        }
        return paints;
